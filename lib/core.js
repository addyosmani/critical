 ...
 * @returns {promise}
 */
function validateStylesheets(htmlfile, stylesheets, opts) {
	if (stylesheets.length === 0) {
		return Promise.reject(new Error('No usable stylesheets found in html source. Try to specify the stylesheets manually.'));
	}
	...
	htmlfile.allErrors = [];
	htmlfile.stylesheetContent = {};
	var promiseArray = [];
	stylesheets.forEach(function(stylesheet) {
		promiseArray.push(new Promise((resolve, reject) => {
			file.requestAsync(stylesheet).then(function(response) {
				if (response.statusCode >= 200 && response.statusCode < 400) {
					htmlfile.stylesheetContent[stylesheet] = response.body;
					...
					result.messages.forEach(function(resultMessage) {
						if (resultMessage.type == 'error') {
							...
							delete resultMessage.evidence; // for minified css, this usually results in a stupidly large string
							delete resultMessage.rule; // unnecessary
							htmlfile.allErrors.push(resultMessage);
						}
					});
				} else {
					htmlfile.stylesheetContent[stylesheet] = '';
				}
				resolve();
			});
		}));
	});
	return Bluebird.all(promiseArray).then(function() {
		if (htmlfile.allErrors.length < 1) {
	        return Bluebird.map(stylesheets, stylesheet => file.assertLocalString(htmlfile.stylesheetContent[stylesheet], 'css')).then(stylesheets => {
				htmlfile.stylesheets = stylesheets;
				return htmlfile;
	        });
		}
		else {
			return Promise.reject(new Error(JSON.stringify(htmlfile.allErrors)));
		}
	});
}
